...
glm::vec3 targetPosition = glm::vec3(4.0f, 6.0f, 0.0f);
...
//            cout << "Child Position = " << child.position.x << " " << child.position.y << " " << child.position.z << endl;
        ...
        if (glfwGetKey( window, GLFW_KEY_P ) == GLFW_PRESS){
            child.update(-0.1f, vec3(0,0,0));
        }
        if (glfwGetKey( window, GLFW_KEY_L ) == GLFW_PRESS){
            child.update(0.1f, vec3(0,0,0));
        }
        if (glfwGetKey( window, GLFW_KEY_O ) == GLFW_PRESS){
            child2.update(-0.1f, vec3(0,0,0));
        }
        if (glfwGetKey( window, GLFW_KEY_K ) == GLFW_PRESS){
            child2.update(0.1f, vec3(0,0,0));
        }
        if (glfwGetKey( window, GLFW_KEY_C ) == GLFW_PRESS){
            child.ModelMatrix = glm::translate(child.ModelMatrix, glm::vec3(translateAmount, 0.0f, 0.0f));
            child.MVP = root.ModelMatrix * ViewMatrix * ProjectionMatrix;
            translateAmount += 0.001f;
            glm::vec3 currBonePosition = glm::vec3(glm::column(root.ModelMatrix, 3).x, glm::column(root.ModelMatrix, 3).y, glm::column(root.ModelMatrix, 3).z);
...
//                cout << "Outermost joint vector " << outerMostJointVector.x << " " << outerMostJointVector.y << " " << outerMostJointVector.z << endl;
                ...
                finger1.bones[i]->update(angle, vec3(0,0,0));
...
void playAnimation(Bone &bone,mat4 ProjectionMatrix, mat4 ViewMatrix, float deltaTime)
{
    ...
    bone.update(sin(time*15)/5, vec3(0,0,0));
